# Brainstorm Session (Structured Output)
...
## HPA Failure Code Routing

### Code Map (`print_failure_hint`) Scope
...
### Deterministic Next Command Map (`print_next_command_for_code`)
...
### 3-Attempt Variant Matrix (All `HPA-301..306`)

| Code | Attempt 1 (Canonical) | Attempt 2 (Tuned) | Attempt 3 (Fallback) |
|---|---|---|---|
| HPA-301 | Apply canonical HPA manifest | Server-side re-apply + target ref tune | Delete/recreate from known-good preset |
| HPA-302 | Apply metrics-server + wait | Restart with KinD-tuned args | Recreate from pinned local KinD manifest |
| HPA-303 | Re-apply deployment with requests | Patch CPU/memory requests directly | Apply tiny-safe fallback deployment preset |
| HPA-304 | Start configured load mode | Deterministic mode switch (`pod<->host`) | Start fallback low internal load profile |
| HPA-305 | Apply proof preset + rerun load | Increase load + extend ramp | Lower HPA target (demo-safe) + host-load fallback |
| HPA-306 | Stop load + observe longer | Patch scale-down behavior + observe | Enforce zero-load + extended observation |

---

## Architecture and Workflow Decisions (Implementation-Level)

- **Gate engine:** `run_gate(name, severity, fn)` wrapper with timing, logs, JSON artifacts, and severity-aware branching
- **HPA proof contract:** baseline -> load start -> scale-up required -> load stop -> cooldown observed
- **Artifacts:** per-run and per-gate JSON/log files, plus HPA-specific evidence files (`describe`, `yaml`, samples CSV, summary)
- **Fix dispatcher:** deterministic `fix.sh CODE` with one code-in, one bounded path out, then standardized resume command
- **Resume path:** `up.sh --resume hpa_proof` executes deterministic mini-sequence (integrity -> reachability -> hpa_proof -> evidence)
- **Outcome synchronization:** Node-based single resolver computes both `overall_status` and `next_command`; all scorecard/next-action output reads the same resolved snapshot
...
## Generated Solution Matrices (Expanded)
...
### `fix.sh` Dispatcher (Deterministic)
...
### `up.sh --resume hpa_proof` Deterministic Flow
...
### `print_final_scorecard` (Synced With Resolver)
...
## Generated Solutions (Consolidated)

- **Solution A: Deterministic gate engine**
  - Implement `run_gate` with severity-aware behavior and per-gate artifacts.
- **Solution B: HPA proof + bounded remediation**
  - Implement `gate_hpa_proof`, code map `301-306`, and 3-attempt escalation matrix.
- **Solution C: Single-source outcome computation**
  - Use Node resolver as only authority for `overall_status` and `next_command`.
- **Solution D: Resume-safe operations**
  - Implement `up.sh --resume hpa_proof` deterministic mini-sequence.
- **Solution E: Learner-first evidence and docs model**
  - Enforce output + screenshot/GIF + checklist artifacts and A/B-first content flow.
