...
_fix_304_attempt_2() {
  ...
  ./scripts/load.sh --mode "${switched_mode}"
  ./scripts/load.sh --status | grep -q "active"
}
...
_fix_305_attempt_2() {
  local load_mode="${LOAD_MODE:-pod}"
  echo "[fix-305] Extending ramp window to 300s + increasing concurrency..."
  mkdir -p .state
  echo "HPA_RAMP_SEC=300" >> .state/env-overrides
  ./scripts/load.sh --preset hpa-proof --concurrency 50 --mode "${load_mode}"
}
...
_fix_305_attempt_3() {
  ...
  echo "HPA_RAMP_SEC=300" >> .state/env-overrides
  ./scripts/load.sh --preset hpa-proof --mode host
}
